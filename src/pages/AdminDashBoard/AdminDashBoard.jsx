...
const AdminDashboard = () => {
  ...
  const fetchAdminData = async () => {
    try {
      setLoading(true);
      // Fetch applications
      const appsResponse = await fetch('https://byvs-backend.onrender.com/api/admin/office-bearer-applications?approved=false', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
        }
      });
      ...
      const tasksResponse = await fetch('https://byvs-backend.onrender.com/api/admin/all-tasks', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
        }
      });
  ...
  const approveApplication = async (id) => {
    try {
      const response = await fetch(`https://byvs-backend.onrender.com/api/admin/office-bearer-applications/${id}/approve`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
  ...
  const createTask = async (taskData) => {
    try {
      const response = await fetch('https://byvs-backend.onrender.com/api/admin/tasks', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
          'Content-Type': 'application/json'
